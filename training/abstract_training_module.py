...
class AbstractGraspAgentTraining:
    ...
    def initialize(self):
        ...
        self.Ave_max_prop = MovingRate(self.model_key + '_Ave_max_prop',
                                                       decay_rate=0.01,
                                                       initial_val=1.,load_last=True,track_history=self.track_statistics_history)
        ...
        self.balanced_set_grasp_safety_statistics = TrainingTracker(name=self.model_key + '_balanced_set_grasp_safety',
                                                            track_label_balance=False,track_history=self.track_statistics_history)

        self.cond_argmax_policy_statistics = TrainingTracker(name=self.model_key + '_cond_argmax_policy',
                                                            track_label_balance=False,track_history=self.track_statistics_history)

        self.argmax_policy_statistics = TrainingTracker(name=self.model_key + '_argmax_policy',
                                                            track_label_balance=False,track_history=self.track_statistics_history)
        ...
        self.grasp_safety_statistics = TrainingTracker(name=self.model_key + '_grasp_safety',
                                                        track_label_balance=False, decay_rate=0.01,track_history=self.track_statistics_history)
    ...
    def pose_interpolation(self, gripper_pose, annealing_factor):
        ...
        if self.cip_fingers is not None: ref_pose=self.cip_fingers(ref_pose)
    ...
    def print_pairs_info(self, pairs, grasp_pose, grasp_pose_ref):
        for j in range(len(pairs)):
            ...
            if k < 0:
                print(Fore.GREEN,
                      target_ref_pose.cpu().numpy()[0:8],
                      target_generated_pose.cpu().detach().numpy()[0:8],
                      ', m=', margin,
                      ', is_col=', is_col,
                      ',  q_score=', q_score,
                      ', c_score=', c_score,
    ...
    def get_repulsive_loss_pi_two_B(self,depth,grasp_pose,features,mask,weight):
    ...
    def step_policy(self, cropped_local_point_clouds, depth, clean_depth, floor_mask, pc, grasp_pose_ref, pairs     ):
        ...
        if print_details: print(Fore.LIGHTYELLOW_EX,
              f'grasp_sampling_loss={grasp_sampling_loss.item()},  grasp_quality_loss_={grasp_quality_loss_}, collision_loss_={collision_loss_.item()}',
              Fore.RESET)
    ...
    def get_grasp_collision_loss(self,coll_probs,grasp_collision_logits,mask_,pc,grasp_pose_PW,random_sampling=False):
        ...
        for k in range(n):
            ...
            while True:
                ...
                coll_free=not(contact_with_obj or contact_with_floor)

                # if time.time() - start > 5 * s:
                #     return torch.tensor(0., device=device)
                label = torch.ones_like(grasp_prediction_logits) if coll_free else torch.zeros_like(
                ...
                if self.force_balance and  time.time() - start < 5 * s:
                    if coll_free and positive_counter >= s: continue
                    if (not coll_free) and negative_counter >= s: continue
            ...
            with torch.no_grad():
                self.balanced_set_grasp_safety_statistics.loss = grasp_quality_loss.item()
                self.balanced_set_grasp_safety_statistics.update_confession_matrix(label.detach(),
                                                                                    logits_to_probs(
                                                                                        grasp_prediction_logits.detach()))
    ...
    def sample_contrastive_pairs(self, pc, floor_mask, grasp_pose, grasp_pose_ref,
        ...
        selection_p =torch.rand_like(grasp_quality) #if self.loaded_synthesised_data is None else grasp_quality*grasp_feasiblity
        ...
        d_pairs = sorted(d_pairs, key=lambda x: x[9], reverse=True)[:self.batch_size]
        g_pairs = sorted(g_pairs, key=lambda x: x[9], reverse=True)[:self.batch_size]
    ...
    def step(self, report=False):
        ...
        if (self.skipped_last or self.skip_rate.val>0.8 or len(self.DDM) < 100) and (not (self.train_policy_only  and not self.explore_mode )) and not self.test_mode :
        ...
        with torch.no_grad():
            ...
            if grasp_poses is not None:
                ...
                if grasp_poses_1.numel() > 0:
                    ...
                    print(f'High prob grasp_pose parameters std = {pose_std2}' )
                    pose_range2=(torch.max(grasp_poses_2, dim=0)[0]-torch.min(grasp_poses_2, dim=0)[0]).cpu()
                    print(f'High prob grasp_pose parameters range = {pose_range2}')
            ...
            self.grasp_safety_statistics.print()

            self.cond_argmax_policy_statistics.print()
            self.argmax_policy_statistics.print()
    ...
    def save_statistics(self):
        ...
        self.grasp_safety_statistics.save()
        ...
        with torch.no_grad() :
            ...
            for i in range(iterations):
                ...
                with torch.no_grad():
                    self.gan.generator.eval()
                    grasp_pose, grasp_quality_logits, features2,features3, grasp_collision_logits = self.gan.generator(
                        depth[None, None, ...], detach_sampler=True,detach_quality=True,detach_collision=True)
