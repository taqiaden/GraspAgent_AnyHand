...
class PoseSampler(nn.Module):
    def __init__(self,n_joint=0):
        ...
        self.delta = FilmModulatedDecoder(in_c1=64, in_c2= 1, out_c=3,activation=nn.LeakyReLU(0.2), normalize=False).to(device)

        self.alpha = FilmModulatedDecoder(in_c1=64, in_c2= 1+3, out_c=3,activation=nn.LeakyReLU(0.2), normalize=False).to(device)
        self.beta = FilmModulatedDecoder(in_c1=64, in_c2= 4+3, out_c=2,activation=nn.LeakyReLU(0.2), normalize=False).to(device)

        self.fingers=FilmModulatedDecoder(in_c1=64, in_c2=9, out_c=n_joint,activation=nn.LeakyReLU(0.2),normalize=False).to(device) if n_joint>0 else None
