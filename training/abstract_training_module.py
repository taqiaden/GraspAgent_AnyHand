...
class AbstractGraspAgentTraining:
    def __init__(self, args,sampler_policy_model,critic_model,  epochs=1 ,model_key='test',
        ...
        self.balanced_set_grasp_value_statistics = None
    ...
    def initialize(self):
        ...
        self.sampled_grasp_quality_statistics = TrainingTracker(name=self.model_key + '_sampled_grasp_quality',
                                                            track_label_balance=True,track_history=self.track_statistics_history)
        self.balanced_set_grasp_value_statistics = TrainingTracker(name=self.model_key + '_balanced_set_grasp_value',
    ...
    def load_optimizers(self):
        ...
        policy_params += list(self.gan.generator.value.parameters())
    ...
    def step_discriminator(self, cropped_local_point_clouds, depth,  gripper_pose, gripper_pose_ref, pairs):
    ...
    def get_generator_loss(self, cropped_local_point_clouds, depth, clean_depth, gripper_pose, gripper_pose_ref, pairs, floor_mask,
                           latent_vector):
        ...
        return loss
    def supplemetary_statistics(self,masked_quality,pc,gripper_pose_PW,floor_mask):
        gripper_target_index = masked_quality.argmax()
        gripper_target_point = pc[gripper_target_index]
        gripper_prediction_ = masked_quality[gripper_target_index].squeeze()
        ...
        label = torch.ones_like(gripper_prediction_) if grasp_success else torch.zeros_like(
            gripper_prediction_)
        self.argmax_grasp_quality_statistics.update_confession_matrix(label.detach(),
                                                                          gripper_prediction_.detach())

        dist = MaskedCategorical(probs=masked_quality,mask=~floor_mask)
    ...
    def step_policy(self, cropped_local_point_clouds, depth, clean_depth, floor_mask, pc, gripper_pose_ref, pairs,
                       latent_vector):
        '''zero grad'''
        ...
        '''generated grasps'''
        gripper_pose, grasp_quality_logits ,features,grasp_value_logits= self.gan.generator(
            depth[None, None, ...])
        ...
        grasp_value_logits = grasp_value_logits[0, 0].reshape(-1)
        masked_quality = logits_to_probs(grasp_quality_logits).clone()
        masked_quality[~floor_mask] = float(-1.)

        gripper_quality_loss_=self.get_grasp_quality_loss(grasp_quality_logits,floor_mask,pc,gripper_pose_PW)

        gripper_value_loss_=self.get_grasp_value_loss( grasp_value_logits, floor_mask, pc, gripper_pose_PW)

        self.supplemetary_statistics( masked_quality, pc, gripper_pose_PW,floor_mask)

        diversity_loss=torch.tensor([0.],device='cuda')
        gripper_sampling_loss=torch.tensor([0.],device='cuda')
        if len(pairs) == self.batch_size:
            gripper_sampling_loss = self.get_generator_loss(cropped_local_point_clouds,
        ...
        loss = gripper_sampling_loss  + gripper_quality_loss_ +diversity_loss+gripper_value_loss_
    ...
    def get_grasp_quality_loss(self,grasp_quality_logits,floor_mask,pc,gripper_pose_PW):

        gripper_quality_loss_ = torch.tensor(0., device='cuda')
        ...
        for k in range(n):
            '''grasp quality'''
            while True:
                gripper_target_index = random_sampling(torch.rand_like(grasp_quality_logits),
                                                       mask=~floor_mask.detach())

                gripper_target_point = pc[gripper_target_index]
                gripper_prediction_logits = grasp_quality_logits[gripper_target_index].squeeze()
                gripper_target_pose = gripper_pose_PW[gripper_target_index].detach()
                ...
                self.grasp_quality_statistics.update_confession_matrix(label.detach(),
                                                                       logits_to_probs(gripper_prediction_logits.detach()))
            ...
            if grasp_success:
                ...
                negative_counter += 1
            label = torch.ones_like(gripper_prediction_logits) if grasp_success else torch.zeros_like(gripper_prediction_logits)
            grasp_quality_loss = c_loss(gripper_prediction_logits, label)
            ...
            with torch.no_grad():
                ...
                self.balanced_set_grasp_quality_statistics.update_confession_matrix(label.detach(),
                                                                           logits_to_probs(
                                                                               gripper_prediction_logits.detach()))

            gripper_quality_loss_ += grasp_quality_loss / n

        return gripper_quality_loss_

    def get_grasp_value_loss(self,grasp_value_logits,floor_mask,pc,gripper_pose_PW):

        gripper_value_loss_ = torch.tensor(0., device='cuda')
        ...
        for k in range(n):
            '''grasp quality'''
            while True:
                gripper_target_index = random_sampling(torch.rand_like(grasp_value_logits),
                                                       mask=~floor_mask.detach())

                gripper_target_point = pc[gripper_target_index]
                gripper_prediction_logits = grasp_value_logits[gripper_target_index].squeeze()
                gripper_target_pose = gripper_pose_PW[gripper_target_index].detach()

                grasp_success, initial_collision, n_grasp_contact, self_collide, stable_grasp, warning_flag, plan_found, grasped_obj = self.evaluate_grasp(
                    gripper_target_point, gripper_target_pose, view=False,
                    shake=False, check_kinematics=False,
                    update_obj_prob=logits_to_probs(gripper_prediction_logits).item() )
            ...
            if grasp_success:
                ...
                negative_counter += 1
            label = torch.ones_like(gripper_prediction_logits) if grasp_success else torch.zeros_like(gripper_prediction_logits)
            gripper_value_loss = c_loss(gripper_prediction_logits, label)


            with torch.no_grad():
                self.balanced_set_grasp_value_statistics.loss = gripper_value_loss.item()
                self.balanced_set_grasp_value_statistics.update_confession_matrix(label.detach(),
                                                                           logits_to_probs(
                                                                               gripper_prediction_logits.detach()))
                
            gripper_value_loss_ += gripper_value_loss / n

        return gripper_value_loss_
    ...
    def sample_contrastive_pairs(self, pc, floor_mask, gripper_pose, gripper_pose_ref,
        ...
        grasp_value = grasp_value[0, 0].reshape(-1)
        ...
        selection_p = torch.rand_like(
            grasp_value)*grasp_value.clamp(min=0.001)   if  len(self.DDM)<self.max_scenes else torch.rand_like(grasp_value)
        ...
        while t < n:
            ...
            gen_value = grasp_value[target_index]
            ...
            gen_success, gen_initial_collision, gen_n_grasp_contact, gen_self_collide, stable_gen_grasp, warning_flag, gen_plan_found, gen_grasped_obj = self.evaluate_grasp(
                target_point, target_generated_pose, view=False, shake=False, check_kinematics=self.check_kinematics,
                update_obj_prob=gen_value.item())
            ...
            if gen_success:
                ...
                if importance is None or not ref_success or grasp_value[target_index].item()>importance:

                    importance = max(0.01,
                                     grasp_value[target_index].item())
                ...
                if (importance is not None and importance>0.1) or len(self.DDM)<self.max_scenes:
                    importance = 0.5*importance if importance is not None else grasp_value[target_index].item()
                    all_pairs.append(
                        (target_index, target_point, gripper_pose_ref_PW[target_index], importance, ref_grasped_obj))
            ...
            if len(d_pairs) < self.batch_size and not (ref_success and gen_success):
                ...
                margin=1-grasp_value[target_index]
        ...
        self.update_synthesised_dat(all_pairs,pc)

        return d_pairs, g_pairs, sampler_samples

    def update_synthesised_dat(self,all_pairs,pc):
        if len(all_pairs) > 0:
            '''Update dynamic data'''
    ...
    def step(self, i, report=False):
        ...
        for k in range(self.iter_per_scene):
            ...
            with torch.no_grad():
                ...
                gripper_pose, grasp_quality_logits,features,grasp_value_logits = self.gan.generator( depth[None, None, ...],detach_backbone=True)
                ...
                if self.loaded_synthesised_data is not None:
                    ...
                    for t in range(len(self.loaded_synthesised_data.target_indexes)):
                        ...
                        if pose.shape==gripper_pose_ref[index].shape:

                            gripper_pose_ref[index] = pose
                        elif pose.shape[0]>=5:
                            gripper_pose_ref[index][0:5] = pose[0:5]
                ...
                if not self.train_policy_only:
                    ...
                    d_pairs, g_pairs, sampler_samples = self.sample_contrastive_pairs(pc, floor_mask, gripper_pose,
                                                                                      gripper_pose_ref,
                                                                                     grasp_quality.detach(),grasp_value.detach() )
    ...
    def view_result(self, gripper_poses, floor_mask):
        with torch.no_grad():
            ...
            self.balanced_set_grasp_value_statistics.print()
    ...
    def save_statistics(self):
        ...
        self.balanced_set_grasp_value_statistics.save()
    ...
    def clear(self):
        ...
        self.balanced_set_grasp_value_statistics.clear()
